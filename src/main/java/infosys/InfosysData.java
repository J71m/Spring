...
public class InfosysData{
    ...
    static private String dbURN = "//greeny.cs.tlu.ee/if17_timjaan";
    ...
    static public String readContactsFromDB()throws Exception{
        ...
        ResultSet countQuery=st.executeQuery("SELECT COUNT(id) AS cCount FROM veeb_contacts"); 
        while (countQuery.next()){count = countQuery.getInt("cCount");}
        ResultSet rs=st.executeQuery("SELECT id, fname, lname, code, phone FROM veeb_contacts ORDER BY id");
    ...
    static public String addContact(String newContact)throws Exception{
        ...
        Connection cn=DriverManager.getConnection("jdbc:mysql:"+dbURN+dbUser+dbPW);
        PreparedStatement st=cn.prepareStatement("INSERT INTO veeb_contacts VALUES (DEFAULT, ?, ?, ?, ?)");
